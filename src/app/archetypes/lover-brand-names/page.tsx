...
const archetypeTitle = 'Lover';

export async function generateMetadata(): Promise<Metadata> {
  const title = `${archetypeTitle} Brand Archetype Names`;
  const meta = buildVibeMeta('elegant'); // Base meta
  meta.title = title;
  meta.description = `Discover passionate, elegant brand names for the ${archetypeTitle} archetype. Perfect for beauty, luxury, and intimacy-focused startups.`;
  meta.slug = `/archetypes/${archetype}-brand-names`;
  return resolveMetadata(meta);
}

const faqs = [
  { question: `What is the ${archetypeTitle} brand archetype?`, answer: `The ${archetypeTitle} brand archetype is all about intimacy, elegance, and sensory pleasure. These brands strive to make their customers feel special, desired, and appreciated.` },
  { question: `How to name a ${archetypeTitle} brand?`, answer: `Naming a ${archetypeTitle} brand involves using soft, elegant, and romantic sounds. Words that evoke passion, beauty, and connection work best (e.g., Victoria's Secret, Godiva).` }
];

export default function ArchetypeNamesPage() {
  const staticNames = generateNames({ industry: 'beauty', vibe: 'elegant', length: 'short', style: 'brandable', isTwoWords: false }, 12);
  const title = `${archetypeTitle} Brand Archetype Names`;
        ...
        <section className="w-full py-16 md:py-24 px-4 bg-muted/20 border-b">
          <div className="container max-w-4xl mx-auto text-center space-y-6">
            <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight text-foreground">
              {title} – Elegant, Passionate, and Intimate
            </h1>
            <p className="text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto">
              Creating a brand built on connection and beauty? Discover names that capture the {archetypeTitle} archetype.
            </p>
            <div className="pt-4">
              <Link href={`/?vibe=elegant`}>
